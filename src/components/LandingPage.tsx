...
export default function LandingPage({ email, onComplete }: LandingPageProps) {
  ...
  useEffect(() => {
    ...
    // Get path point and angle for ladybird positioning
    const getPathPoint = (progress: number) => {
      const distance = Math.min(pathLength * progress, pathLength);
      const point = svgPath.getPointAtLength(distance);
      
      // Get next point for angle calculation
      const nextDistance = Math.min(distance + 5, pathLength);
      const nextPoint = svgPath.getPointAtLength(nextDistance);
      
      const angle = Math.atan2(nextPoint.y - point.y, nextPoint.x - point.x);
      
      // Get SVG bounding rect to scale coordinates to canvas space
      const svgBBox = svgPath.ownerSVGElement?.getBoundingClientRect();
      
      // Scale SVG coordinates to canvas coordinates
      // The SVG viewBox is 0 0 1105.765 85
      // Map to the actual displayed SVG size
      const svgWidth = 1105.765;
      const svgHeight = 85;
      
      if (svgBBox) {
        // Calculate scale factors
        const scaleX = svgBBox.width / svgWidth;
        const scaleY = svgBBox.height / svgHeight;
        
        // Transform SVG coordinates to screen coordinates
        const screenX = svgBBox.left + point.x * scaleX;
        const screenY = svgBBox.top + point.y * scaleY;
        
        // Convert to canvas coordinates (canvas is full screen at 0,0)
        return { 
          x: screenX, 
          y: screenY, 
          angle 
      ...
      // Fallback if bounding rect is not available
      return { x: canvas.width / 2, y: canvas.height / 2, angle };
    };
    ...
    const animate = () => {
      ...
      // Clear canvas (transparent, not blue)
      ctx!.clearRect(0, 0, canvas.width, canvas.height);

      if (progress < 0.75) {
        // Walking phase - ladybird follows the text path
        const walkProgress = progress / 0.75;
        const pathPoint = getPathPoint(walkProgress);
        
        drawLadybird(pathPoint.x, pathPoint.y, 35, pathPoint.angle);
      } else {
        // Circling phase
        const circleProgress = (progress - 0.75) / 0.25;
        
        // Get position at end of path (the O in DOT)
        const endPoint = getPathPoint(1);
        
        // Circle around with spiral inward
        const spiralRadius = 100 * (1 - circleProgress * 0.8);
        const circleAngle = circleProgress * Math.PI * 3;
        
        const ladybirdX = endPoint.x + Math.cos(circleAngle) * spiralRadius;
        const ladybirdY = endPoint.y + Math.sin(circleAngle) * spiralRadius;
        ...
        if (circleProgress > 0.9) {
          drawLadybird(endPoint.x, endPoint.y, 30, 0);
        }
      }
  ...
  return (
    ...
    >
      {/* Visible SVG with the text path - ladybird will follow this */}
      <svg
        ref={svgContainerRef}
        style={{
          position: "absolute",
      ...
      >
        {/* Text path - visible white stroke that appears to be written */}
        <path
          ref={svgPathRef}
